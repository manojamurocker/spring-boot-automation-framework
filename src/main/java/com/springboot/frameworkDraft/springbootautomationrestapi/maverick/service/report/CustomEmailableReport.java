package com.springboot.frameworkDraft.springbootautomationrestapi.maverick.service.report;
...
public class CustomEmailableReport implements IReporter {
    ...
    private String dReportTitle = "Store Front B2B API Automation - User Journeys";
    private String dReportFileName = "emailable-report.html";

    List<ISuite> testSuites;
    ...
    protected void writeStylesheet() {
        ...
        writer.print("table {margin-bottom:10px;border-collapse:collapse;empty-cells:show; border: solid 1px #000000; width: 1200px}");
        writer.print("#summary {margin-top:30px}");
        writer.print("h1 {font-size:30px}");
        writer.print("Body {width:100%;}");
        writer.print("th,td {padding: 4px; border: solid 1px #000000;}");
        ...
        writer.print(".passedodd td, .pass {background-color: #0A0}");
        writer.print(".passedeven td, .stripe .pass {background-color: #469446}");
        writer.print(".skippedodd td, .yellow {background-color: #fae457}");
        writer.print(".skippedeven td, .stripe .yellow {background-color: #e6d149}");
    ...
    protected void writeSuiteSummary() {
        ...
        float totalPassPercentage=0;
        float totalFailPercentage=0;
        float totalSkipPercentage=0;

        writer.println("<div class=\"easy-test-overview\">");
        writer.println("<table class=\" easy-overview\">");
        writer.print("<tr>");
        ...
        for (SuiteResult suiteResult : suiteResults) {
            ...
            for (TestResult testResult : suiteResult.getTestResults()) {
                ...
                writeTableData(integerFormat.format(passedTests), "num pass");
                writeTableData(integerFormat.format(skippedTests),
                        (skippedTests > 0 ? "num yellow" : "num"));
        ...
        totalPassPercentage = Precision.round((((float)totalPassedTests)/((float)totalTestsCount))*100, 2);
        totalFailPercentage = Precision.round((((float)totalFailedTests)/((float)totalTestsCount))*100, 2);
        totalSkipPercentage = Precision.round((((float)totalSkippedTests)/((float)totalTestsCount))*100, 2);
        ...
        if (testIndex > 1) {
            ...
            writeTableHeader(integerFormat.format(totalPassedTests) +" ["+totalPassPercentage+"%]", "num pass");
            writeTableHeader(integerFormat.format(totalSkippedTests) +" ["+totalSkipPercentage+"%]",
                    (totalSkippedTests > 0 ? "num yellow" : "num"));
            writeTableHeader(integerFormat.format(totalFailedTests) +" ["+totalFailPercentage+"%]",
    ...
    protected void writeErrorSummary() {
        ...
        int testIndex = 0;
        Map<String, Integer> failedCountClassWise = new HashMap<>();
        int totalFailedCount = 0;
        for (SuiteResult suiteResult : suiteResults) {
            for (TestResult testResult : suiteResult.getTestResults()) {
                List<ClassResult> classResultList = testResult.failedTestResults;
                int failedTestCount = testResult.getFailedTestCount();
                totalFailedCount += failedTestCount;
                for(ClassResult classResult : classResultList){
                    String className = classResult.className;
                    className = className.substring(className.lastIndexOf(".")+1);
                    System.out.println("ClassName:"+ className);
                    List<MethodResult> methodResultList = classResult.methodResults;
                    for(MethodResult methodResult : methodResultList){

                        List<ITestResult> testResultList = methodResult.results;
                        System.out.println(testResultList.size());

                        for(ITestResult iTestResult : testResultList){

                            String mapKey = "["+className+"] "+ iTestResult.getMethod().getMethodName();
                            if(failedCountClassWise.containsKey(mapKey)){
                                failedCountClassWise.put(mapKey, failedCountClassWise.get(mapKey)+1);
                            }else
                                failedCountClassWise.put(mapKey, 1);
                        }
                    }
                }
        ...
        writer.println("<table class=\" easy-overview\">");
        writer.print("<tr>");
        writer.print("<th>[Test Class] Method</th>");
        writer.print("<th>Total Failed Cases</th>");
        writer.print("<th>Percentage</th>");
//        writer.print("<th># Skipped</th>");
//        writer.print("<th># Failed</th>");
//        writer.print("<th>Included Groups</th>");
//        writer.print("<th>Excluded Groups</th>");
...
//        writer.print("<th># Total<br/>Time(hh:mm:ss)</th>");
        ...
        System.out.println("failedCountClassWise:"+failedCountClassWise);
        System.out.println("totalFailed Count:"+totalFailedCount);

        int k=0;
        for(Map.Entry entry : failedCountClassWise.entrySet()){
            writer.print("<tr");
            if ((k % 2) == 1) {
                writer.print(" class=\"stripe\"");
            }
            ...
            writeTableData(buffer.append("<a href=\"#t").append(testIndex)
                    .append("\">")
                    .append(Utils.escapeHtml(entry.getKey().toString()))
                    .append("</a>").toString());
            writeTableData(integerFormat.format(entry.getValue()), (Integer.parseInt(entry.getValue().toString()) > 0 ? "num attn" : "num"));
            double percentage = Precision.round((Double.parseDouble(entry.getValue().toString())/totalFailedCount) * 100, 2);
            writeTableData(percentage+"%", (percentage > 0 ? "num attn" : "num"));
    ...
    protected void writeScenarioSummary() {
        ...
        writer.print("<table class=\" easy-overview\" id='summary'>");
        writer.print("<thead>");
        writer.print("<tr>");
